 ...
 * @author rohan
 */
public class NextWord extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            HttpSession session;
             ...
             userCatModel ucm=(userCatModel)session.getAttribute("ucm");
             UserCatDao ucd=new UserCatDao();
            if(submit[0].equals("PREVIOUS CATEGORY"))
            {
                ucm.setCategory(ucm.getCategory()-1);
                session.setAttribute("ucm",ucm);
                
            }
            else if(submit[0].equals("NEXT CATEGORY"))
            {
              ucd.updateCategory(ucm, context,session);
              session.setAttribute("ucm",ucm);
